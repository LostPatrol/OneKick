...
const release = { tag_name: '0.1.4', body: '# FIX\n\n+ Fixed kicking.', draft: false, prerelease: true };
const asset = (mc, v = '0.1.4') => ({ id: mc === '1.20.1' ? 1 : 2,
  name: `onekick-${mc}-${v}.jar`, state: 'uploaded', size: 100 });
...
test('single NeoForge release only creates NeoForge destinations', () => {
  ...
  assert.ok(rows.every(r => r.version === '1.21.1-0.1.4'));
});
test('single Forge release only creates Forge destinations', () => {
  const rows = planRelease(release, [asset('1.20.1')]).include;
  assert.equal(rows.length, 2);
  assert.ok(rows.every(r => r.minecraft === '1.20.1' && r.loader === 'forge' && r.java === '17'));
});
...
test('branch versions come from each asset, including different mod versions', () => {
  const rows = planRelease(release, [asset('1.20.1'), asset('1.21.1', '0.1.5')]).include;
  assert.equal(rows[2].version, '1.21.1-0.1.5');
});
...
test('stable GitHub releases map to release channel', () => {
  assert.equal(planRelease({ ...release, prerelease: false }, [asset('1.21.1')]).include[0].versionType, 'release');
});
test('source and documentation artifacts never become production uploads', () => {
  assert.equal(planRelease(release, [asset('1.21.1'), { name: 'onekick-1.21.1-0.1.4-sources.jar' },
...
test('ambiguous and foreign production JARs fail before uploads', () => {
  assert.throws(() => planRelease(release, [asset('1.20.1'), asset('1.20.1', '0.1.5')]), /Multiple/);
...
function snapshotFixture(t, overrides = {}) {
  ...
  return { written, download, getReleaseByTag, args: { core,
    context: { repo: { owner: 'LostPatrol', repo: 'OneKick' }, eventName: 'release', payload: { release } },
...
test('snapshot downloads each JAR once and preserves exact changelog and bytes', async t => {
  ...
  assert.equal(f.written.get('release-assets/onekick-1.21.1-0.1.4.jar').toString(), 'release-file-bytes');
...
test('manual dispatch retrieves the specified historical Release', async t => {
  ...
  process.env.RELEASE_TAG = '0.1.4';
  f.args.context.eventName = 'workflow_dispatch';
  try {
    await prepareRelease(f.args);
    assert.equal(f.getReleaseByTag.mock.calls[0].arguments[0].tag, '0.1.4');
  } finally {
