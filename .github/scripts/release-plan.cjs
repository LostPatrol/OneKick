...
function planRelease(release, assets, { branch = 'all', platform = 'all' } = {}) {
  ...
  for (const asset of assets) {
    ...
    const match = /^onekick-(1\.20\.1|1\.21\.1)-(\d+\.\d+\.\d+(?:-[A-Za-z0-9.-]+)?)\.jar$/.exec(asset.name);
    if (!match) throw new Error(`Unrecognized production JAR: ${asset.name}`);
    const [, minecraft, modVersion] = match;
    ...
    seen.add(minecraft);
    builds.push({ minecraft, ...targets[minecraft], version: `${minecraft}-${modVersion}`,
