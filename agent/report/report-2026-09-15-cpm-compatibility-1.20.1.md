# CPM compatibility implementation report (1.20.1)

## Scope

- Branch: `1.20.1` (Minecraft 1.20.1 / Forge).
...
- Registered a small client-only plugin through CPM's supported Forge IMC API.
...
- Third-person model replacement, networking, kick mechanics, particles, sound, and
  server code are unchanged.

## Verification

- `gradlew compileJava --no-daemon`: passed.
- `gradlew test build --no-daemon`: passed, including Forge `reobfJar`.
- `git diff --check`: passed; only expected Windows LF-to-CRLF checkout notices appeared.
